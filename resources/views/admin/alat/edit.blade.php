...
<div class="max-w-2xl">
    <div class="bg-gradient-to-br from-indigo-600/40 to-indigo-800/40 backdrop-blur-sm border border-indigo-500/30 rounded-xl p-6 shadow-lg mb-6">
        <h1 class="text-2xl font-bold text-indigo-300">✏️ Edit Alat</h1>
    </div>
        ...
        <form action="{{ route('admin.alat.update',$alat->id) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <select name="kondisi" class="w-full p-3 bg-slate-800 border border-slate-700 rounded-lg text-white focus:outline-none focus:border-indigo-500 transition"
                    ...
                    <option value="diperbaiki" {{ $alat->kondisi == 'diperbaiki' ? 'selected' : '' }}>Diperbaiki</option>
                </select>
                @error('kondisi')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="flex gap-3 pt-4">
                <button type="submit" class="flex-1 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold py-3 rounded-lg transition">
                    ✓ Update Alat
                </button>
